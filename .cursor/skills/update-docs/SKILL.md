...
1. Start `bun run dev`, then screenshot with Playwright (`playwright-core` is in node_modules; Chrome at `/usr/bin/google-chrome-stable`) using a `1280x720` viewport, `deviceScaleFactor: 1`. Stage a good shot: relevant app/pane open (`http://localhost:5173/<app-id>` launches an app directly), wait ~20s for boot + entrance animations.
2. Convert: `sharp(png).webp({ quality: 80 }).toFile(...)` and verify metadata is 1280×720.
3. The Vite watcher ignores `public/**` — restart the dev server before visually verifying new assets, or they 200 with the SPA HTML fallback.
