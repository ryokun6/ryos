...
import { getClientIp, makeKey, checkRateLimit } from "../_middleware/rate-limit.js";
import { validateAuthToken } from "../_middleware/auth.js";
...
const rateLimitExceededResponse = (
  ...
  identifier: string,
  result: { count: number; limit: number; allowed: boolean; windowSeconds: number; resetSeconds: number; remaining: number }
): Response => {
  const resetSeconds =
...
export default async function handler(req: Request): Promise<Response> {
  ...
  if (usernameHeader) {
    const validationResult = await validateAuthToken(usernameHeader, authToken);
