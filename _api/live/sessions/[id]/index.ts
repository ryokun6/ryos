 ...
 * Fetch Live Desktop session state
 */
...
export default apiHandler(
  ...
  async ({ req, res, logger, startTime }) => {
    ...
    try {
      assertValidRoomId(sessionId, "live-desktop-session-get");
    } catch (error) {
      logger.response(400, Date.now() - startTime);
      res
        .status(400)
        .json({ error: error instanceof Error ? error.message : "Invalid session ID" });
      return;
    }
    ...
    try {
      ...
      logger.info("Live desktop session fetched", { sessionId });
      logger.response(200, Date.now() - startTime);
      res.status(200).json({ session });
      return;
    } catch (error) {
      logger.error("Failed to fetch live desktop session", error);
