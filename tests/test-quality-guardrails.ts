...
export async function runQualityGuardrailTests(): Promise<{
  ...
  await runTest("fails when spaced shell:true is introduced", async () => {
    const qualityRoot = withTempQualityRoot((root) => {
      mkdirSync(join(root, "scripts"), { recursive: true });
      writeFileSync(
        join(root, "scripts", "BadSpacedShell.ts"),
        `const opts = { shell : true };\nconsole.log(opts);\n`,
        "utf-8"
      );
    });
    ...
    try {
      ...
      assertEq(
        result.status,
        1,
        `Expected failure exit code 1 for spaced shell:true usage, got ${result.status}`
      );
      assert(
        (result.stdout || "").includes("FAIL shell:true command execution"),
        "Expected shell:true guardrail failure for spaced key syntax"
      );
    } finally {
      rmSync(qualityRoot, { recursive: true, force: true });
    }
  ...
  await runTest("fails when spaced computed shell:true key is introduced", async () => {
    const qualityRoot = withTempQualityRoot((root) => {
      mkdirSync(join(root, "scripts"), { recursive: true });
      writeFileSync(
        join(root, "scripts", "BadComputedShellSpaced.js"),
        'const opts = { [ "shell" ] : true };\nconsole.log(opts);\n',
        "utf-8"
      );
    });
    ...
    try {
      ...
      assertEq(
        result.status,
        1,
        `Expected failure exit code 1 for spaced computed shell:true key, got ${result.status}`
      );
      assert(
        (result.stdout || "").includes("FAIL shell:true command execution"),
        "Expected shell:true guardrail failure for spaced computed key syntax"
      );
    } finally {
      rmSync(qualityRoot, { recursive: true, force: true });
    }
  ...
  await runTest("passes when similar non-shell key is used", async () => {
    const qualityRoot = withTempQualityRoot((root) => {
      mkdirSync(join(root, "scripts"), { recursive: true });
      writeFileSync(
        join(root, "scripts", "SafeSubshellKey.ts"),
        `const opts = { subshell : true };\nconsole.log(opts);\n`,
        "utf-8"
      );
    });
    ...
    try {
      ...
      assertEq(
        result.status,
        0,
        `Expected pass exit code 0 for non-shell key name, got ${result.status}`
      );
      assert(
        (result.stdout || "").includes("PASS shell:true command execution"),
        "Expected shell:true guardrail pass for non-shell key name"
      );
    } finally {
      rmSync(qualityRoot, { recursive: true, force: true });
    }
  ...
  await runTest(
    "fails when document spaced bracket write usage is introduced in JavaScript",
    async () => {
      const qualityRoot = withTempQualityRoot((root) => {
        mkdirSync(join(root, "src"), { recursive: true });
        writeFileSync(
          join(root, "src", "BadDocumentSpacedBracketWrite.js"),
          `document ["write"] ("<p>oops</p>");\n`,
          "utf-8"
        );
      });
      ...
      try {
        ...
        assertEq(
          result.status,
          1,
          `Expected failure exit code 1 for document spaced bracket write in js, got ${result.status}`
        );
        assert(
          (result.stdout || "").includes("FAIL document.write usage"),
          "Expected document.write guardrail failure for spaced bracket write variant"
        );
      } finally {
        rmSync(qualityRoot, { recursive: true, force: true });
      }
  ...
  await runTest(
    "fails when document spaced-dot write usage is introduced in JavaScript",
    async () => {
      const qualityRoot = withTempQualityRoot((root) => {
        mkdirSync(join(root, "src"), { recursive: true });
        writeFileSync(
          join(root, "src", "BadDocumentSpacedDotWrite.js"),
          `document . write ("<p>oops</p>");\n`,
          "utf-8"
        );
      });
      ...
      try {
        ...
        assertEq(
          result.status,
          1,
          `Expected failure exit code 1 for document spaced-dot write in js, got ${result.status}`
        );
        assert(
          (result.stdout || "").includes("FAIL document.write usage"),
  ...
  await runTest(
    "fails when template-bracket string-based setTimeout is introduced",
    async () => {
      const qualityRoot = withTempQualityRoot((root) => {
        mkdirSync(join(root, "scripts"), { recursive: true });
        writeFileSync(
          join(root, "scripts", "BadTemplateBracketTimer.js"),
          "globalThis[`setTimeout`](\"console.log('x')\", 100);\n",
          "utf-8"
        );
      });
      ...
      try {
        ...
        assertEq(
          result.status,
          1,
          `Expected failure exit code 1 for template-bracket timer string, got ${result.status}`
        );
        assert(
          (result.stdout || "").includes("FAIL string-based timer execution usage"),
          "Expected string-based timer guardrail failure for template-bracket invocation"
        );
      } finally {
        rmSync(qualityRoot, { recursive: true, force: true });
      }
  ...
  await runTest("fails when merge conflict markers are introduced in mdx", async () => {
    const qualityRoot = withTempQualityRoot((root) => {
      mkdirSync(join(root, "docs"), { recursive: true });
      writeFileSync(
        join(root, "docs", "MergeConflict.mdx"),
        [
          "# Example",
          "<<<<<<< HEAD",
          "<p>left</p>",
    ...
    try {
      ...
      assertEq(
        result.status,
        1,
        `Expected failure exit code 1 for mdx merge markers, got ${result.status}`
      );
      assert(
        (result.stdout || "").includes("FAIL merge conflict markers"),
        "Expected merge conflict marker guardrail failure for mdx"
      );
    } finally {
      rmSync(qualityRoot, { recursive: true, force: true });
    }
  ...
  await runTest("fails when merge conflict markers are introduced in .github workflow", async () => {
    const qualityRoot = withTempQualityRoot((root) => {
    ...
    try {
      ...
      assertEq(
        result.status,
        1,
        `Expected failure exit code 1 for .github merge markers, got ${result.status}`
      );
      assert(
        (result.stdout || "").includes("FAIL merge conflict markers"),
        "Expected merge conflict marker guardrail failure for .github workflow"
      );
    } finally {
      rmSync(qualityRoot, { recursive: true, force: true });
    }
  ...
  await runTest("fails when merge conflict markers are introduced in ini", async () => {
    const qualityRoot = withTempQualityRoot((root) => {
      writeFileSync(
    ...
    try {
      ...
      assertEq(
        result.status,
        1,
        `Expected failure exit code 1 for ini merge markers, got ${result.status}`
      );
      assert(
        (result.stdout || "").includes("FAIL merge conflict markers"),
        "Expected merge conflict marker guardrail failure for ini"
      );
    } finally {
      rmSync(qualityRoot, { recursive: true, force: true });
    }
  ...
  await runTest("fails when merge markers include trailing whitespace", async () => {
    const qualityRoot = withTempQualityRoot((root) => {
      mkdirSync(join(root, "src"), { recursive: true });
      writeFileSync(
        join(root, "src", "merge-conflict-trailing-whitespace.ts"),
        [
          "export const value = 1;",
          "<<<<<<< HEAD   ",
          "left",
    ...
    try {
      ...
      assertEq(
        result.status,
        1,
        `Expected failure exit code 1 for merge markers with trailing whitespace, got ${result.status}`
      );
      assert(
        (result.stdout || "").includes("FAIL merge conflict markers"),
        "Expected merge conflict marker guardrail failure for trailing-whitespace markers"
      );
    } finally {
      rmSync(qualityRoot, { recursive: true, force: true });
    }
  ...
  await runTest("fails when merge conflict markers are introduced in root .gitignore", async () => {
    const qualityRoot = withTempQualityRoot((root) => {
      writeFileSync(
        join(root, ".gitignore"),
        [
    ...
    try {
      ...
      assertEq(
        result.status,
        1,
        `Expected failure exit code 1 for .gitignore merge markers, got ${result.status}`
      );
      assert(
        (result.stdout || "").includes("FAIL merge conflict markers"),
        "Expected merge conflict marker guardrail failure for .gitignore"
      );
    } finally {
      rmSync(qualityRoot, { recursive: true, force: true });
    }
    ...
    async () => {
      ...
      try {
        ...
        assertEq(
          result.status,
          1,
          `Expected failure exit code 1 for hidden-directory merge markers, got ${result.status}`
        );
        assert(
          (result.stdout || "").includes("FAIL merge conflict markers"),
          "Expected merge conflict marker guardrail failure for hidden config directory files"
        );
      } finally {
        rmSync(qualityRoot, { recursive: true, force: true });
      }
