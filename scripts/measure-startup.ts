...
async function readSample(page: Page, theme: string): Promise<StartupSample> {
...
try {
  for (const theme of themes) {
    ...
    samples.push(await readSample(page, theme));
    await context.close();
  }
} finally {
  await browser.close();
...
for (const sample of samples) {
  ...
  console.log(
    `[startup:${sample.theme}] ${sample.scriptRequests} scripts, ${sample.stylesheetRequests} stylesheets, ${sample.imageRequests} images, ${sample.sameOriginTransferKiB.toFixed(1)} KiB transferred`
  );
  console.log(
    `[startup:${sample.theme}] fonts ${sample.fontRequests.join(", ")}`
  );
}

const macosxFonts = samples.find((sample) => sample.theme === "macosx")?.fontRequests;
const system7Fonts = samples.find((sample) => sample.theme === "system7")?.fontRequests;
if (
