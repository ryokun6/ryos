...
- Retrieves the APNs device token (cached across launches once received) and validates token format before forwarding
...
- On logout, the frontend resolves the local APNs token and attempts a token-scoped
  unregister request before auth logout (best effort, with short lookup/request timeouts;
  timed-out unregister requests are aborted when supported).
...
- `bun run test:push:cors-utils` — shared CORS helper suite only (origin/referer parsing, requested-header token filtering/dedupe, preflight vary merge/dedupe, bounded echoed requested headers and scan limits, max-age normalization, optional preflight credential-header suppression, configured method/header normalization)
...
- `bun run test:push-apns-guard` — shared APNs config guard helper tests
