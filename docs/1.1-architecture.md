...
| **Self-hosted API** | Bun (`Bun.serve`) | Run `_api/*` routes without Vercel via `scripts/api-standalone-server.ts` |
...
- **Vercel serverless functions:** `_api/*` handlers run as Node.js functions for primary web deployment.
...
Shared utilities under `_api/_utils/` (including `api-handler.ts`, `middleware.ts`, `request-auth.ts`, `redis.ts`, `constants.ts`, `_cors.ts`, `_ssrf.ts`, `_rate-limit.ts`, `_validation.ts`, `_logging.ts`, and `_sse.ts`) provide consistent API patterns across routes.

Frontend clients under `src/api/` (`auth`, `rooms`, `admin`, `songs`, and `listen`) centralize request logic for app modules.
...
bun run dev:vercel    # Optional Vercel parity/debugging
bun run api:dev       # Standalone API server (no vercel CLI)
bun run dev:standalone # Vite with /api proxy to standalone API
```
