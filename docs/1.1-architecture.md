...
Vite + Bun build system with multiple deployment targets, served by the standalone Bun API server, plus optimized chunk splitting for performance.
...
graph TB
    ...
    subgraph API["API Layer (standalone Bun server)"]
...
|--------|------------|-------------|
| **Web (PWA)** | Docker / Coolify | Primary deployment (standalone Bun server) |
...
| **Self-hosted API** | Bun (`Bun.serve`) | Runs `api/*` routes via `scripts/api-standalone-server.ts` |
...
- [Self-hosting on VPS](/docs/self-hosting-vps) - Run API/frontend on your own server
