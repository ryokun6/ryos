...
Shared utilities under `api/_utils/` (including `api-handler.ts`, `middleware.ts`, `request-auth.ts`, `redis.ts`, `constants.ts`, `_cors.ts`, `_ssrf.ts`, `_rate-limit.ts`, `_validation.ts`, `_logging.ts`, and `_sse.ts`) provide consistent API patterns across routes.
