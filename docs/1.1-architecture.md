...
Frontend clients under `src/api/` (`auth`, `rooms`, `admin`, `songs`, and `listen`) centralize request logic for app modules; the Cloud Sync v2 client engine lives in `src/sync/`.
