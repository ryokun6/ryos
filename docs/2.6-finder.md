...
Finder integrates seamlessly with other ryOS applications; for instance, files can be opened by their respective default applications (e.g., a text file by [TextEdit](/docs/textedit)), and newly installed applications from the [Applet Store](/docs/applet-store) automatically appear in the Applications folder.
