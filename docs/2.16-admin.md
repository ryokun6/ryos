...
*   **Server Status Monitoring**: A dedicated Server page showing system version, commit hash, deployment environment detection (Vercel, Coolify, local), Redis connectivity, WebSocket/Pusher status, a dashboard server card, and Cursor agent counts.
...
- `src/apps/admin/components/ServerPanel.tsx`: Server status monitoring panel showing deployment environment, Redis, and WebSocket status.
