...
│   ├── Desktop.tsx
│   ├── Dock.tsx          # Public entry: re-exports macOS dock when theme has dock
│   ├── dock/             # MacDock implementation (see Dock section below)
│   ├── MenuBar.tsx
│   ├── WindowFrame.tsx
│   ├── AppleMenu.tsx
│   ├── StartMenu.tsx
...
macOS X (Aqua) dock. The public shell import stays **`Dock`** from `@/components/layout/Dock`; Windows themes render a taskbar instead and **`Dock` returns `null`**.

#### Public API invariants

- **`export function Dock()`** — only entry point imported by `AppManagerView`. Do not import `MacDock` from app code.
- Theme gating uses **`useThemeFlags().isMacOSTheme`**; dock visibility/height for layout math still comes from **`getThemeMetadata()`** (`hasDock`, `baseDockHeight`).
- Pinned/open state and preferences live in **`useDockStore`** (`src/stores/useDockStore.ts`), persisted under `dock-storage`. Protected pins: `finder`, `__applications__`, `__trash__`.
- Swipe-to-reveal on mobile shares **`dockRevealGesture.ts`** with Dashboard shell guards (`useDashboardShellInputDisabled`).

#### Module layout (`src/components/layout/dock/`)

| File | Role |
|------|------|
| `MacDock.tsx` | Orchestration: open/pinned lists, trash drop, auto-hide, layout animations |
| `DockPinnedItems.tsx` / `DockOpenItems.tsx` | Pinned strip and running-app / applet instances |
| `DockIconButton.tsx` | Single icon: magnification, indicators, drag, intent prefetch |
| `DockApplicationsButton.tsx` / `DockTrashButton.tsx` | Launcher folder and Trash targets |
| `DockDivider.tsx` / `DockSpacer.tsx` | Separator and magnify spacer slots |
| `dockConstants.ts` | `DOCK_*` sizing/magnification; `DOCK_MULTI_WINDOW_APPS` |
| `dockTypes.ts` | Shared props (`DockIconButtonProps`, `DockOpenItem`, …) |
| `dockContextMenus.ts` | Menu item builders (pin, hide, open, multi-window) |
| `useDockContextMenus.ts` | Context-menu state wired to `MacDock` |
| `useDockDragDrop.ts` | Reorder pins, file/applet drops, trash highlight |
| `useDockMagnification.ts` | Framer `mouseX` motion + scale curve |
| `useDockIconHover.ts` | Hover/swap state for open icons |
| `dockTrashHandlers.ts` | Trash empty / delete-on-drop handlers |
| `dockAppletInfo.ts` | Resolve applet file metadata for dock file items |

#### Changing dock behavior

1. **New pinned app defaults** — update `DEFAULT_PINNED_ITEMS` in `useDockStore.ts` (respect `PROTECTED_DOCK_ITEMS`).
2. **Multi-window from dock** — add `AppId` to `DOCK_MULTI_WINDOW_APPS` in `dockConstants.ts` and ensure context menus in `dockContextMenus.ts` expose “New Window” where appropriate.
3. **Icon interactions** — prefer extending `DockIconButton` / subcomponents; keep store writes in `MacDock` or hooks, not in presentational children.
4. **Magnification / size** — tune `DOCK_MAX_SCALE`, `DOCK_MAGNIFY_DISTANCE`, `DOCK_BASE_BUTTON_SIZE`; user scale still flows from `useDockStore.scale`.
5. **Cloud sync of pins** — dock domain in `src/sync/domains.ts` reads/writes `useDockStore`; do not add parallel persistence.

**Features (unchanged behavior, split implementation):**
- Icon magnification on hover (toggle via dock preferences)
- Running app indicators and open applet/file instances
- Pinned items: drag-to-reorder, add from Finder/applets, context menus
- Trash drop target and empty-trash confirm
- Auto-hide dock with bottom-edge hover / mobile swipe reveal
