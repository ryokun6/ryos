...
## App-specific labels in Aqua dark mode

Avoid hard-coded Tailwind tints such as `text-purple-700/70` or `text-orange-600` on Chats sidebar meta (IRC pill, unread counts, online counts). On **`macosx` + `data-os-color-scheme="dark"`**, those colors fall below contrast on `bg-neutral-100` / selected rows.

**Pattern (Chats IRC label — merged on branch `cursor/fix-chat-irc-label-dark-mode-15bf`, apply the same approach for new badges):**

1. Give the element a stable class (e.g. `chat-room-irc-label`) in the component; drop theme-specific Tailwind color utilities on that node.
2. Define base + selected + theme overrides in **`src/styles/themes.css`** near other Chats rules (after `.chat-bubble` blocks):
   - Default: `color: rgb(126 34 206 / 0.7);`
   - Selected row: `[data-selected="true"] .chat-room-irc-label { color: rgba(255, 255, 255, 0.75) !important; }`
   - Aqua light: `:root[data-os-theme="macosx"] .chat-room-irc-label { … }`
   - Aqua dark (scoped to window content): `:root[data-os-theme="macosx"][data-os-color-scheme="dark"] .window-body .chat-room-irc-label { color: #c4b5fd !important; }`
3. Unread badges in `ChatRoomSidebar.tsx` still use `text-orange-600` for emphasis; if contrast regresses in dark mode, move them to a named class with the same `.window-body` dark override pattern instead of adding more `dark:` Tailwind branches in TSX.

Reuse **`--os-color-text-primary`** / **`--os-color-text-secondary`** when the label is body text, not a semantic accent.
