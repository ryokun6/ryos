...
`ipodControl` (target "music"), `karaokeControl` (target "karaoke"), and `tvControl` (target "tv") remain registered as deprecated aliases with the same per-target validation.
