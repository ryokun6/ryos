...
requires the current conversation ID and matching revision. It is accepted only
while legacy import remains open and the server conversation is empty or
contains only proactive greetings; it is rejected once substantive history
exists or after that conversation has been reset. Reset requires the current
