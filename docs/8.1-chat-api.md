...
Each message can retain up to 128,000 text characters and 48 parts.

The durable part allowlist includes text, owned image references, tool state,
and source metadata. Reasoning and provider metadata are not retained. Tool
inputs are limited to 16 KiB and outputs to 64 KiB, except `generateHtml`
outputs, which may use 512 KiB. Oversized payloads and sensitive results such
as memory reads or local file contents are replaced with an omission marker.
Historical tool state is restored for rendering but is not replayed to the
model; current tool continuations are still overlaid for the active turn.
