...
revision, and keeps at most 200 text messages within a 256 KiB text budget.
Reasoning, provider metadata, images, and tool payloads are not retained; a rich
file or tool message is accepted only as the current action needed by that turn.
