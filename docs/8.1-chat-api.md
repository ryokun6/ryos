...
interface ChatRequest {
  // AI model to use (optional, defaults to "gpt-6")
  model?: SupportedModel;
  // Required together with the ryo auth account to call any non-default model
  debugMode?: boolean;
...
|----------|----------|------------------|
| `gpt-6` | OpenAI | gpt-6-astra (default) |
| `gpt-5.5` | OpenAI | gpt-5.5 (ryo + debug only) |
| `opus-5.5` | Anthropic | claude-opus-5-5 (ryo + debug only) |

### Model Selection

```typescript
// Priority: Query param > Body param > Default
const model = queryModel || bodyModel || "gpt-6";
```
