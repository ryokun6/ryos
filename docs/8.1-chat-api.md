...
| POST | `/api/ai/attachments` | Prepare or complete an authenticated image upload |
| GET | `/api/ai/attachments/:id` | Redirect an authenticated owner to an image download |
...
5 MiB after client preprocessing. Reset and account deletion remove registered
