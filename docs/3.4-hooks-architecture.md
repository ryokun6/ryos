...
ryOS provides 37 custom hooks organized by functionality, enabling audio playback, window management, media handling, background synchronization, and more.
...
**Features:**
- Posts index/query messages to `spotlightSearch.worker.ts`
- Defers initial indexing with `requestIdleCallback` for responsiveness
- Subscribes to files/music/sites/video stores and refreshes index incrementally
