...
ryOS provides 38 custom hooks organized by functionality, enabling audio playback, window management, media handling, background synchronization, and more.
...
Realtime cloud sync orchestration via Pusher or local WebSocket. Subscribes to sync channels for each domain (files, settings, songs, videos, stickies, calendar, contacts) and applies remote updates in real time. Used by `App.tsx` for authenticated users with auto-sync enabled.
