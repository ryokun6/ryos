...
ryOS supports **two object storage backends** for cloud sync and private AI
conversation images. The API validates uploaded images with the production
`sharp` dependency before storing them.
...
Block public access to the bucket. AI attachments are read only through the
authenticated ryOS API. If bucket versioning is enabled, add a lifecycle rule
that expires noncurrent versions so deleted conversation images do not remain
as old object versions.
