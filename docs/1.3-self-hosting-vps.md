...
### Optional Internet Explorer proxy enhancements

The default IE proxy works without extra services. These variables enable heavier browser features for self-hosted deployments that want them:

```bash
IE_PROXY_SESSIONS=1
HEADLESS_RENDER_URL_TEMPLATE="https://renderer.example.com/render?url={url}"
HEADLESS_RENDER_AUTH_HEADER="Authorization"
HEADLESS_RENDER_AUTH_TOKEN="..."
HEADLESS_BROWSER_WS_ENDPOINT="wss://chrome.example.com?token=..."
IE_LIVE_BROWSER=1
IE_LIVE_VIEW_URL_TEMPLATE="https://browser.example.com/live?url={url}"
```

Use `IE_PROXY_SESSIONS=1` only when you want the proxy to keep a per-host cookie jar in Redis for embedded sites. Headless and live-browser providers are optional escape hatches for bot-blocked, JavaScript-heavy, or fully interactive sites.
...
```bash
# STORAGE_PROVIDER may be omitted; S3 is auto-detected from the S3_* variables.
S3_BUCKET=ryos-sync
S3_REGION=us-east-1
...
# Optional: set STORAGE_CLIENT_UPLOAD to server-mediated uploads when bucket CORS is unavailable.
```
...
If presigned browser uploads still fail because the bucket service does not return CORS headers on presigned `PUT` responses, set `STORAGE_CLIENT_UPLOAD` to server-mediated uploads so uploads go through `/api/sync/v2/blob-upload` instead of directly to object storage.
