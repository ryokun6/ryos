# Self-hosting ryOS on a VPS or Coolify

ryOS runs as a **single Bun production server**. That server handles:
...
ryOS uses **S3-compatible object storage** for cloud sync and private AI chat images. <!-- pragma: allowlist secret -->
...
The workflow POSTs to `COOLIFY_WEBHOOK_URL` (Coolify's `/api/v1/deploy` is POST-only; GET returns HTTP 405). It also exposes a manual `workflow_dispatch` trigger with a `force_deploy` input that appends `force=true` to the webhook URL (useful when only environment variables changed).
