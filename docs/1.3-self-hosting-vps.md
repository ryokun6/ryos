...
- `yt-dlp` on `PATH` (or set `YT_DLP_PATH`) for the iPod / Karaoke / Videos / TV YouTube playback pipeline. The bundled `Dockerfile` installs it automatically; on a plain VPS see [§ "yt-dlp dependency"](#yt-dlp-dependency).
...
- `YT_DLP_PATH` — absolute path to the `yt-dlp` binary if it isn't on `PATH`

### yt-dlp dependency

The YouTube extractor (`/api/youtube/extract` + `/api/youtube/stream`) shells out to **yt-dlp** to resolve playable stream URLs. The built-in `Dockerfile` already pulls the standalone Linux binary from GitHub (pinned via the `YT_DLP_VERSION` build arg). On a plain VPS install it once with:
...
`ffmpeg` is **not** required — the API only uses `yt-dlp -J --skip-download` to get the JSON dump, never to mux files. If `yt-dlp` is missing the endpoint returns `503 yt-dlp binary not available on the server`.
