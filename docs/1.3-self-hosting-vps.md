...
S3_PROXY_BLOBS=true        # optional: stream blob up/downloads through the API (skip bucket CORS)
```

#### Avoiding bucket CORS with `S3_PROXY_BLOBS`

Cloud sync stores per-item blobs (images, wallpapers, trash, applets) and
manual backups in object storage. By default the browser uploads/downloads
those blobs **directly** to the bucket using presigned URLs, which requires the
bucket's **CORS policy** to allow every origin the app is served from
(`https://your-domain.com`, the Electron desktop app, `localhost` during dev,
etc.). If the bucket CORS allowlist doesn't include the active origin, uploads
fail with:

> Upload failed before an HTTP response was received for `https://…/sync/…`.
> Check bucket CORS, endpoint reachability, and TLS/mixed-content configuration.

Set `S3_PROXY_BLOBS=true` to route blob uploads **and** downloads through the
same-origin API (`/api/sync/blob-proxy`) instead. The server streams the bytes
to/from object storage with server-side credentials, so cloud sync works from
any origin without touching the bucket's CORS configuration.

> ⚠️ Only enable this on hosts **without** small request-body limits (plain
> Bun, Coolify, VPS, containers). It is **not** suitable for Vercel Serverless
> Functions, which cap request bodies at ~4.5MB — on Vercel, use Vercel Blob
> (CORS is handled for you) or configure bucket CORS and leave this unset.
...
S3_ACCESS_KEY_ID=your-access-key
S3_SECRET_ACCESS_KEY=your-secret-key
S3_PROXY_BLOBS=true   # recommended unless the bucket CORS allows your app origin(s)
```
