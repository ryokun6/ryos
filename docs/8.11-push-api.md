...
(trimmed, case-insensitively deduplicated, preserving first-seen casing).
Invalid requested header-name tokens are ignored before echoing.
When request header values are repeated, values are merged before normalization.
Echoed requested headers are bounded to the first 50 valid names (max 128 chars each).
Preflight requested-header parsing scans at most the first 200 comma-separated candidates.
When repeated `Access-Control-Request-Headers` values are present, only the first 50
header-value entries are scanned (blank entries count toward this entry scan limit).
Allowed-origin preflight requests that declare a non-`POST` request method are rejected with `405 Method not allowed`.
Requested preflight method values are token-validated and capped at 32 characters.
When repeated `Access-Control-Request-Method` values are present, only the first 20
method-value entries are scanned (blank entries count toward this entry scan limit).
In that rejection path, requested preflight headers are not echoed.
Disallowed-origin preflight requests are rejected with `403 Unauthorized` before requested-method checks
and do not include `Allow` headers.
When requested preflight headers are missing/empty/invalid, push endpoints fall back to
...
(`Origin` always; preflight responses additionally vary on
...
- Body must be a JSON object.
- Tauri iOS frontend helper normalizes/validates APNs token format before submission;
  backend validation remains authoritative and always enforced.
- Tauri iOS frontend notification helper trims alert text fields and suppresses
  foreground toasts for silent payloads (no alert title/body content).
...
- Omitting `token` performs account-wide cleanup for that user (all registered tokens).
  For device-scoped logout, pass the current device token.
- Current iOS wrapper logout flow resolves the local APNs token and sends token-scoped unregister when available.
  Token lookup and unregister request are both best effort and bounded by short timeouts
  to avoid blocking logout UX. Timed-out unregister requests are aborted when supported
  by the runtime.
...
Run token helper suites (shared format + normalization + parity):

```bash
bun run test:push:token
```

Run only shared token-format helper suite:

```bash
bun run test:push:token-shared
```

Run only shared CORS helper suite:

```bash
bun run test:push:cors-utils
```

This suite validates shared `_api/_utils/_cors` behavior used by push-adjacent API
routes, including:
- Origin/referer header precedence and trimming
- Preflight `Vary` header merge/dedup behavior
- Requested preflight header-token filtering (valid CORS token chars, name-length caps, case-insensitive dedupe)
- Bounded repeated requested-header scanning and echoed-header size limits
  (50 scanned values, 200 token candidates, 128-char token-name cap,
  1024 chars per value, 4096-char aggregate echo cap)
- CORS max-age normalization (`0` preserved; invalid/negative values fallback to `86400`)
- Optional credential-header suppression (`credentials: false`) for shared preflight handlers
- Configured CORS method/header arrays are trimmed + case-insensitively deduped;
  empty/invalid configured arrays fallback to safe defaults

Run only frontend/backend token-parity helper suite:

```bash
bun run test:push:token-parity
```

Run only logout-token helper suite:

```bash
bun run test:push:logout
```

Run only Tauri push helper suite:

```bash
bun run test:push:tauri
```
...
bun run test:push-token-parity
bun run test:push-logout
bun run test:push-tauri-notifications
...
Run the token-helper validation bundle (lint + shared format + normalization + parity suites) with:

```bash
bun run check:push:token
```

Run the token-parity validation bundle (lint + token-parity suite) with:

```bash
bun run check:push:token-parity
```

Run the shared token-format validation bundle (lint + shared token suite) with:

```bash
bun run check:push:token-shared
```

Run the shared CORS-helper validation bundle (lint + CORS helper suite) with:

```bash
bun run check:push:cors-utils
```

Run the logout-helper validation bundle (lint + logout helper suite) with:

```bash
bun run check:push:logout
```

Run the Tauri-helper validation bundle (lint + Tauri helper suite) with:

```bash
bun run check:push:tauri
```
...
Run the guard-focused validation bundle (lint + CORS/request/auth/config guard suites) with:
