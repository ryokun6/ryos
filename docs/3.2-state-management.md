...
| Store | Persistence Key | Purpose |
|-------|-----------------|---------|
| `useChatsStore` | `ryos:chats` (IndexedDB) | Chat rooms, messages, authentication — persisted to IndexedDB (`persisted_state` store) because AI tool results can exceed the localStorage quota |
...
| `useTextEditStore` | `ryos:textedit` (IndexedDB) | Open documents and unsaved-draft recovery |
...
| `useStickiesStore` | `stickies-storage` (IndexedDB) | Sticky notes and note layout |
| `useCalendarStore` | `calendar-storage` | Calendar events, todo items, groups |
| `useContactsStore` | `contacts-storage` (IndexedDB) | Address book contacts, vCard import/export |
...
const STORES = {
  ...
  PERSISTED_STATE: "persisted_state", // Large Zustand slices
};
...
Quota-sensitive stores use `createIndexedDBPersistStorage()`. The shared
adapter debounces and serializes writes, participates in backup flushing, and
migrates the same-named legacy localStorage value on first hydration. Because
IndexedDB hydration is asynchronous, default seeding and sync jobs wait for
...
```mermaid
flowchart TB
    subgraph "Metadata Layer (IndexedDB persisted_state)"
