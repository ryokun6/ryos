...
| `useSoundboardStore` | `ryos:soundboard` (IndexedDB) | Soundboard slots and recordings — persisted to IndexedDB (`persisted_state` store) instead of localStorage because base64 audio overflows the localStorage quota |
...
const STORES = {
  ...
  PERSISTED_STATE: "persisted_state", // Large Zustand persist slices
};
...
### IndexedDB-backed Zustand persist

Stores that can exceed the per-origin localStorage quota use `createIndexedDBPersistStorage()` from `src/utils/indexedDBPersistStorage.ts`. The adapter keeps Zustand's `persist` shape but writes the serialized slice into the `persisted_state` object store with debounced write-behind semantics.

Soundboard uses this path for recorded audio clips. On first read, the adapter migrates a legacy localStorage value into IndexedDB and removes the old key only after the copy succeeds. Pending writes flush on `pagehide` and tab-hidden events through the shared persist write queue.

Manual backups call `settlePersistWrites()` before reading browser storage so queued localStorage and IndexedDB persist writes are included. Backup format v4 includes the `persisted_state` object store, which keeps Soundboard recordings and any future large persisted slices in local `.gz` exports.
