...
| `usePhotoBoothStore` | `ryos:photo-booth` | Recent photos |
| `usePcStore` | `ryos:pc` | DOS game catalog |
...
| `useInfinitePcStore` | `ryos:store:infinite-pc` | Virtual PC runtime state |
| `useMapsStore` | `ryos:maps:v1` | Maps favorites and home/work settings |
| `useWeatherStore` | `ryos:weather` | Weather lookup preferences |
...
All production TypeScript callers should import storage keys from
`src/config/storageKeys.ts`. The registry groups active keys by owner, separates
legacy aliases, and exposes dynamic builders for per-app handoff keys and
per-user sync state keys.

Current audit findings:

| Bucket | Current shape | Notes |
|--------|---------------|-------|
| Zustand stores | `STORE_STORAGE_KEYS` | Shipped names remain stable to preserve local user data. New store keys should use `ryos:store:<owner>` unless migrating an existing key. |
| Preferences | `LOCAL_STORAGE_KEYS.theme`, `LOCAL_STORAGE_KEYS.language` | Scalar settings should use `ryos:prefs:<domain>:<name>` for new domains; current theme/language names are preserved for compatibility. |
| Caches | `LOCAL_STORAGE_KEYS.cache` | New cache keys should use `ryos:cache:<owner>:<name>:vN` and be safe to delete. |
| Handoffs | `LOCAL_STORAGE_KEYS.handoff` | New one-shot app handoffs should use `ryos:handoff:<target>:<name>`; existing Finder/TextEdit keys dual-read legacy aliases. |
| Migrations | `LOCAL_STORAGE_KEYS.migrations` | Migration flags should use `ryos:migration:<domain>:<name>:vN` for new migrations. |
| Session-only | `SESSION_STORAGE_KEYS` | New per-tab keys should use `ryos:session:<owner>:<name>`. |
| Legacy aliases | `LEGACY_LOCAL_STORAGE_KEYS` | Do not introduce new unprefixed keys; dual-read old keys, write the active key, then remove the old key. |

The registry also prevents active-key collisions. `usePcStore` keeps the shipped
`ryos:pc` DOS game key, while `useInfinitePcStore` now uses
`ryos:store:infinite-pc` because it had no meaningful persisted payload and was
previously colliding with `usePcStore`.
...
setTheme: (theme) => {
  set({ current: theme });
  localStorage.setItem(LOCAL_STORAGE_KEYS.theme.current, theme);
  localStorage.removeItem(LEGACY_LOCAL_STORAGE_KEYS.theme.current);
...
hydrate: () => {
  const saved =
    localStorage.getItem(LOCAL_STORAGE_KEYS.theme.current) ??
    localStorage.getItem(LEGACY_LOCAL_STORAGE_KEYS.theme.current);
