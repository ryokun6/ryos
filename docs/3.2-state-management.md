...
ryOS uses Zustand for state management with 21+ stores following consistent patterns, combined with localStorage and IndexedDB for persistence.
...
|-------|-----------------|---------|
| `useAppStore` | `ryos:app-store` | Window/instance management, boot state, AI model selection, recent apps/documents, expose mode |
