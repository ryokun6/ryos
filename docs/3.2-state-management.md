...
ryOS uses Zustand for state management with 25 stores following consistent patterns, combined with localStorage and IndexedDB for persistence. CPU-intensive search indexing (Spotlight dynamic results) is offloaded to a dedicated Web Worker rather than running on the main thread.
...
`useSpotlightSearch` builds snapshots from `useFilesStore`, `useIpodStore`, `useInternetExplorerStore`, and `useVideoStore`, then posts indexing/query requests to `spotlightSearch.worker.ts`. Initial indexing is deferred with `requestIdleCallback` (fallback timeout), which keeps first-render interactions responsive while search data is prepared in the background.
