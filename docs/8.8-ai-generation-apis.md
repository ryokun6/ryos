...
```typescript
{
  ...
  // Optional Ryo Studio draft mode
  studio?: {
    action: "create" | "edit";
    currentHtml?: string;    // required for action="edit"
    title?: string;
    icon?: string;
    name?: string;
    windowWidth?: number;    // 240-1200
    windowHeight?: number;   // 240-1200
  };
}
```
...
**Ryo Studio Draft Mode**

```json
{
  "reply": "I created a compact tea timer draft with preset brew buttons.",
  "applet": {
...
Ryo Studio draft create/edit requests currently use the same text-generation model with structured output plus a text fallback repair pass when needed.
...
**Ryo Studio Create Draft**

```bash
curl -X POST https://os.ryo.lu/api/applet-ai \
  -H "Content-Type: application/json" \
  -d '{
    "prompt": "Build a tiny tea timer with preset brew buttons",
    "studio": {
      "action": "create"
    }
  }'
...
**Ryo Studio Refine Draft**

```bash
curl -X POST https://os.ryo.lu/api/applet-ai \
  -H "Content-Type: application/json" \
  -d '{
    "prompt": "Add a black tea button and make the timer display bigger",
    "studio": {
      "action": "edit",
      "currentHtml": "<style>...</style><div>...</div><script>...</script>",
      "title": "Tea Timer",
      "icon": "🍵",
      "name": "Tea Timer",
      "windowWidth": 360,
