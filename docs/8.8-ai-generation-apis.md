...
- **Text and image**: `gpt-6` (`gpt-6-astra`)
...
## Stuff Cover Background Removal

Removes the background from a Stuff inventory cover image and returns a shelf-ready transparent PNG cutout (grounded subject, transparent alpha) for shelf display.

### Endpoint

| Method | Path | Description |
|--------|------|-------------|
| POST | `/api/stuff/remove-background` | Edit cover image → transparent PNG |

### Request

```typescript
{
  imageBase64: string;  // raw base64 or data-URL payload (max ~4 MB decoded)
  mediaType: string;    // image/png | image/jpeg | image/webp | image/gif
}
```

Auth is **required** (cookie session or `Authorization` + `X-Username`).

### Response

Binary PNG (`Content-Type: image/png`) with alpha. On failure, JSON `{ error: string }`.

The Stuff client stores the result with `coverPresentation: "cutout"` (transparent stage + drop shadow). Shelf JSON export/import and public shares preserve that field; transparent PNG/WebP covers also auto-detect as cutouts at display time when the flag is unset.

### Rate Limits

| User Type | Limit | Window |
|-----------|-------|--------|
| Authenticated | 10/hour | 1 hour |
| `ryo` | Bypass | — |

### Model

OpenAI `gpt-image-1` via official OpenAI SDK `images.edit` (`background: "transparent"`, `output_format: "png"`). Multipart upload uses `toFile` so the image part includes a filename (required by `/v1/images/edits`). The edit prompt asks for a shelf-ready cutout (grounded pose, subject near the bottom of the frame) rather than a floating mid-frame knock-out.
...
| Model | Provider |
|-------|----------|
| `gpt-6` (default, API: `gpt-6-astra`) | OpenAI |
| `gpt-5.5` (ryo + debug only) | OpenAI |
| `opus-5.5` (API: `claude-opus-5-5`, ryo + debug only) | Anthropic |
...
Uses `gpt-6` (`gpt-6-astra`) for metadata parsing.
...
- **[API Reference](/docs/api-reference)** - Overview of all ryOS API endpoints
