...
The API uses Node.js route handlers for all endpoints. These handlers run on Vercel serverless functions by default and can also run on the standalone Bun API server for self-hosting.
