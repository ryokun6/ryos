...
ryOS APIs are implemented as Vercel-style Node.js route handlers under `_api/`. They can run either on Vercel serverless functions or via the standalone Bun API server for self-hosted deployments.
...
graph TB
    ...
    subgraph API["Node.js API Runtime"]
        ChatAPI[chat]
        SongAPI[songs]
        SpeechAPI[speech]
        RoomsAPI[rooms]
