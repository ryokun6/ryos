...
├── sync/                      # Cloud sync endpoints (backup, auto-sync, tokens)
...
| `/api/presence/heartbeat` | GET, POST | Global online presence heartbeat / list online users |
...
| `/api/sync/auto` | GET, POST | Auto-sync metadata per domain (with individual item manifests) |
| `/api/sync/auto-token` | POST | Generate upload token for auto-sync |
| `/api/sync/state` | GET, POST | Per-domain sync state (Redis-backed domains) |
...
| `sync:auto:meta:` | Auto-sync metadata | `sync:auto:meta:alice` |
