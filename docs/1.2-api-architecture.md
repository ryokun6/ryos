...
ryOS uses Vercel Edge Functions for its backend API, with AI providers, Redis caching, and real-time features.
...
│       ├── index.ts          # Main auth exports (Edge compatible)
│       ├── _validate.ts      # Token validation
│       ├── _tokens.ts        # Token management
│       ├── _extract.ts       # Request auth extraction
│       ├── _password.ts      # Password hashing (Node.js only - bcrypt)
│       ├── _password-storage.ts # Password storage (Edge compatible)
...
| `POST /api/song/[id]` | Fetch | Body: `{ action: "fetch-lyrics" }` |
| `POST /api/song/[id]` | Translate | Body: `{ action: "translate-stream" }` |
| `POST /api/song/[id]` | Furigana | Body: `{ action: "furigana-stream" }` |
