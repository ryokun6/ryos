...
Email is an **optional** channel gated behind `RESEND_API_KEY` +
`RECOVERY_EMAIL_FROM`. When unset, `email/set` returns `503` and `email/status`
reports `emailConfigured: false`; recovery still works via Telegram.
...
| `RESEND_API_KEY` | Optional — enables the recovery-email channel (Resend) |
| `RECOVERY_EMAIL_FROM` | Optional — `From` address for recovery email (e.g. `ryOS <noreply@os.ryo.lu>`) |
