...
| GET | `/api/cron/sync-maintenance` | Maintenance cron: v1 key retirement + blob GC | `CRON_SECRET` |
...
and force download. Supports `?prefix=` for partial reads. On a user's first
v2 access the server imports their legacy v1 sync state into the key-value
model automatically.
...
across runs via a scan cursor persisted at `sync2:maint:cursor`:

1. **v1 key retirement** — frozen legacy sync keys (`sync:state:*`,
   `sync:auto:meta:*`, `sync:songs:*`) were written without TTLs; once a
   user is imported into v2 they get a 90-day retirement TTL. (Live v2 sync
   data uses the 1-year `USER_TTL_SECONDS`, refreshed on both writes and —
   throttled daily — reads, so any device check-in keeps cloud data alive.)
2. **Blob garbage collection** — content-addressed blobs whose digest is no
...
3. **Legacy object sweep** — v1 per-item and monolithic storage objects
   listed in the frozen `sync:auto:meta` manifests are deleted once no
   current document references them, and the manifest is pruned so deletes
   are never retried.
4. **User record healing** — `chat:users:*` records persist forever; stale
