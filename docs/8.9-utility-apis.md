...
Proxies official Taiwan YouBike 2.0 JSON (no TDX key). Taipei uses the city DOT Azure blob (cached 60s). Other cities use YouBike’s official national 2.0 dump (about 6MB, cached 120s, 12s timeout). A viewport bbox selects intersecting feeds; the national dump is waited on when Taipei is out of view or the camera sticks outside Taipei. Results are then filtered to that bbox.
...
```json
{
  ...
  "cacheHit": true,
  "sources": [{ "id": "taipei", "city": "taipei", "ok": true, "count": 1807 }]
}
```
...
Taipei’s DOT Azure blob is the verified primary feed. Non-Taipei coverage comes from the official national dump (municipal portals for Taichung, New Taipei, Taoyuan, Hsinchu, Tainan, and Kaohsiung are dead or non-JSON).
