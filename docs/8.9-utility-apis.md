...
## Pusher Broadcast API (Internal)

Internal relay endpoint used by server-side components that need to trigger Pusher from Node runtime.

### Endpoint

| Method | Path | Description |
|--------|------|-------------|
| POST | `/api/pusher/broadcast` | Trigger arbitrary Pusher event |
