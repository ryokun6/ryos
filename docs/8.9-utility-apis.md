...
Proxies official Taiwan YouBike 2.0 municipal open-data JSON (no TDX key). Each city feed is cached in Redis for 60 seconds. A viewport bbox selects only intersecting city feeds; optional portals never block Taipei. Results are then filtered to that bbox.
...
Taipei’s DOT Azure blob is the verified primary feed. Other city portals are fetched best-effort.
