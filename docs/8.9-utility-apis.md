...
Proxies official Taiwan YouBike 2.0 JSON (no TDX key). Taipei uses the city DOT Azure blob (cached 60s). Other cities use YouBike’s official national 2.0 dump (about 6MB, cached 120s, 20s timeout). A viewport bbox selects intersecting feeds; the national dump is waited on when Taipei is out of view or the camera sticks outside Taipei. Results are then filtered to that bbox.
