...
Checks if a URL allows iframe embedding and optionally proxies content to bypass embedding restrictions. It also powers Internet Explorer's historical cache, cross-origin subresource proxying, optional cookie sessions, optional headless rendering, and optional live-browser handoff.

### Endpoint

| Method | Path | Description |
|--------|------|-------------|
| GET | `/api/iframe-check` | Check iframe embeddability, proxy content, list caches, or return live-browser URLs |
| POST/PUT/PATCH/DELETE | `/api/iframe-check` | Forward proxied subresource/API requests to the target origin |
...
| `mode` | string | No | Operation mode: `check`, `proxy`, `ai`, `list-cache`, or `live` (default: `proxy`) |
...
| `raw` | boolean | No | Treat the request as a subresource proxy request and stream the upstream payload without HTML rewriting |
| `ieSessions` | boolean | No | Opt into IE proxy cookie/session passthrough for permitted debug/admin callers |
| `render` | string | No | Set to `headless` to force the optional headless renderer for permitted debug/admin callers |
| `dbg` | boolean | No | Marks a request from the IE Debug menu; required for opt-in debug features when not authenticated as admin |
...
Per-host verdicts are cached and surfaced with `X-Embed-Cache`.
...
- Fetch/XHR and asset rewriting so same-site-different-origin subresources flow back through `/api/iframe-check?raw=1`
- Font override styles (unless theme is "macosx")

Top-level HTML responses are rewritten. Raw subresource requests (`raw=1` or any non-GET request) stream the upstream payload without injection, which lets JSON APIs, scripts, styles, images, and form submissions keep their original content type.

If `year` and `month` are provided, the proxy asks the Wayback availability API for the closest matching snapshot before falling back to the constructed archive URL.

Optional debug features:

- Cookie/session passthrough is always on when `IE_PROXY_SESSIONS=1`. Otherwise, admin/debug callers can opt in with `ieSessions=1`. ryOS keeps a first-party `ie_psid` cookie and a per-host cookie jar in Redis. ryOS auth cookies are never forwarded upstream.
- Headless fallback runs automatically for top-level GET requests when a configured provider exists and the upstream blocks the normal proxy with common bot-block/JS-required responses. Admin/debug callers can force it with `render=headless`.
...
| `X-IE-Proxy` | Debug summary for cookies/headless/forced-render decisions |
| `X-Embed-Cache` | Embeddability cache status for check mode |
...
Returns an embeddable live-browser URL for providers that run a full interactive browser outside ryOS. This is off by default. When not configured, the endpoint returns `501`.

**Response (200):**

```json
{
  "liveViewUrl": "https://browser-provider.example/live?target=https%3A%2F%2Fexample.com"
}
```
...
- `medium.com`
- `nytimes.com`, `bbc.com`, `bbc.co.uk`, `theguardian.com`, `cnn.com`, `washingtonpost.com`
- `linkedin.com`, `instagram.com`, `facebook.com`
- `amazon.com`, `youtube.com`, `twitch.tv`, `netflix.com`
...
|----------|-------------|
| `IE_PROXY_SESSIONS` | Enables cookie/session passthrough for all IE proxy requests when set to `1` or `true` |
| `HEADLESS_RENDER_URL_TEMPLATE` | HTTP render-service template for headless HTML fallback. Supports `{url}` and `{rawUrl}` placeholders |
| `HEADLESS_RENDER_AUTH_HEADER` / `HEADLESS_RENDER_AUTH_TOKEN` | Optional auth header pair for the HTTP render service |
| `HEADLESS_BROWSER_WS_ENDPOINT` | Chrome DevTools Protocol WebSocket endpoint for an optional browser renderer |
| `IE_LIVE_BROWSER` | Enables `mode=live` when set to `1` or `true` |
| `IE_LIVE_VIEW_URL_TEMPLATE` | Embeddable live-view URL template. Supports `{url}` and `{rawUrl}` placeholders |
...
| `live` | No mode-specific limiter in this route | N/A |
...
Scheduled maintenance (daily via `vercel.json` crons; authenticated with
