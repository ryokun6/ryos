...
The sync endpoints manage backup metadata and retrieval around client-side Vercel Blob uploads.

### Endpoint Summary

| Method | Path | Description | Auth |
|--------|------|-------------|------|
| POST | `/api/sync/backup-token` | Generate short-lived Blob client upload token | Required |
| POST | `/api/sync/auto-token` | Generate domain-specific auto-sync upload token | Required |
...
| GET, POST | `/api/sync/auto` | Automatic sync with Pusher/WebSocket notifications | Required |
| GET, PUT | `/api/sync/state` | Sync state read/write | Required |

### POST /api/sync/backup-token

Generates a direct-upload token for `backups/{username}/backup.gz`.
...
### POST /api/sync/auto-token

Generates a direct-upload token for domain-specific auto-sync uploads (e.g., contacts, settings). Rate-limited to 20 requests/minute.
...
| Field | Type | Description |
|-------|------|-------------|
| `domain` | string | Sync domain (e.g., `"contacts"`, `"settings"`) |
| `itemKey` | string | Optional item key for individual blob sync domains |
...
```json
{
  "blobUrl": "https://...blob.vercel-storage.com/backups/user/backup.gz",
  "timestamp": "2026-02-29T12:34:56.000Z",
  "version": 3,
  "totalSize": 1234567
}
