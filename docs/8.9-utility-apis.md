...
Approximate city for the current request IP. Reuses the same Redis-cached `resolveIpGeolocation` path as chat, weather, and analytics (`ipwho.is` by default; override with `IP_GEOLOCATION_URL_TEMPLATE`). Cloudflare `CF-IP*` and Vercel `x-vercel-ip-*` headers are used when present so the outbound lookup can be skipped. Maps uses this for the first-open city-wide camera when there is no Home, selected place, or granted GPS.

### Endpoint

| Method | Path | Description |
|--------|------|-------------|
| GET | `/api/geo` | Approximate city latitude / longitude for the request IP |
...
When the lookup has no usable point (private IP, disabled provider, Null Island):
