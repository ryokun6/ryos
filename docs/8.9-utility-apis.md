...
**Notes:**
- List results are sorted with featured applets first, then newest first.
...
|--------|-------|--------|-------|
| List applets (`list=true`) | 60 | 1 minute | per IP |
| Get applet by ID | 120 | 1 minute | per IP |
| Save/update applet | 20 | 1 minute | per user |
| Delete applet | 10 | 1 minute | per admin user |
| Patch applet | 10 | 1 minute | per admin user |
...
Admin requests are limited to **30 requests per minute** per admin user.
