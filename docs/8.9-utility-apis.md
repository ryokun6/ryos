...
Proxies the official YouBike 2.0 national dump (`station-yb2.json`, the same file the YouBike app loads). One source covers every city, including Taichung. The ~6MB payload is cached 120s (Redis + process memory) with a 20s fetch timeout. A viewport bbox selects intersecting coverage; results are then filtered to that bbox.

### Endpoint

| Method | Path | Description |
|--------|------|-------------|
| GET | `/api/youbike/stations` | Live YouBike 2.0 stations for the Maps overlay |
...
When any bbox field is present, all four must form a valid box (`south < north`, `west < east`). Omit them to return the full cached set.
...
```json
{
  ...
  "fetchedAt": 1758800000000,
  "cacheHit": true,
  "sources": [{ "id": "national", "city": "unknown", "ok": true, "count": 9629 }]
}
```
...
| 400 | `invalid_bbox` |
| 429 | Rate limit exceeded |
| 502 | Every official feed failed or returned no stations |
...
The official YouBike app dump is the only source. Municipal portals for Taichung, New Taipei, Taoyuan, Hsinchu, Tainan, and Kaohsiung are dead or non-JSON and are not fetched.

---

## YouBike Bike Route

Fallback for on-road YouBike bike geometry when MapKit JS `Directions.Transport.Cycling` (WWDC25) is missing or fails. Proxies the public FOSSGIS OSRM **bike** profile (no API key) with `steps=true` so turn-by-turn street names are available. Walk legs still use MapKit Walking. Results are cached in Redis for 30 minutes. Override the upstream URL with `YOUBIKE_BIKE_ROUTE_URL_TEMPLATE` (`{fromLat}` `{fromLng}` `{toLat}` `{toLng}`) if you self-host a bike router.

### Endpoint

| Method | Path | Description |
|--------|------|-------------|
| GET | `/api/youbike/route` | On-road bike geometry between two Taiwan points |
...
Both points must be inside the Taiwan gate. Hops longer than 50 km are rejected.
...
```json
{
  "path": [
    ...
    { "latitude": 25.0341, "longitude": 121.5639 }
  ],
  "distanceMeters": 6464.4,
...
| Status | Description |
|--------|-------------|
| 400 | `invalid_coordinates`, `not_in_taiwan`, or `too_far` |
| 429 | Rate limit exceeded |
| 502 | Upstream bike router failed or returned a non-road path |
...
| Daily | 400 requests | 24 hours |

---

## Maps Directions

Fallback for in-map **Drive** and **Transit** when MapKit JS `Directions.route` cannot return a path (common for Transit, which is not always on the JS `Transport` enum). Proxies Apple Maps Server API `GET /v1/directions`. Anonymous callers are allowed. Results are cached in Redis for 15 minutes.

### Endpoint

| Method | Path | Description |
|--------|------|-------------|
| GET | `/api/maps/directions` | Automobile or transit route between two points |
...
| `toLat` | number | Yes | Destination latitude |
| `toLng` | number | Yes | Destination longitude |
| `mode` | string | No | `drive` (default) or `transit` |
| `destinationLabel` | string | No | Arrival place name for the route card |

### Response

**Success (200):** a `DirectionsRoutePlan` (`path`, `steps`, `distanceMeters`, `durationSeconds`, `provider: "maps-server"`).
...
| 429 | Rate limit exceeded |
| 502 | `route_unavailable` or `transit_unavailable` |
