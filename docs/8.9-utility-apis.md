...
ryOS provides several utility API endpoints for common operations like fetching link previews, checking iframe embeddability, sharing applets, and admin operations. These endpoints are implemented as Vercel Serverless Functions (Node.js runtime).

## Push Notifications

Authenticated push-notification endpoints are available for the iOS Tauri wrapper:

| Method | Path | Description |
|--------|------|-------------|
| POST | `/api/push/register` | Register current user device token |
| POST | `/api/push/unregister` | Unregister one token or all current user tokens |
| POST | `/api/push/test` | Send APNs test push to current user tokens |

For full payload/response details and environment requirements, see [Push API](/docs/push-api).
