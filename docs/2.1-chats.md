...
### Sidebar labels (themes)

Room list meta (IRC pill, unread / online counts) lives in `ChatRoomSidebar.tsx`. For **Aqua dark mode**, do not rely on fixed Tailwind purple/orange utilities for small badges—use named classes and rules in `src/styles/themes.css` (see [Theme architecture (implementation)](/docs/theme-architecture) → *App-specific labels in Aqua dark mode*). The IRC pill fix uses class `chat-room-irc-label`.
