...
# Add new English keys, then sync missing keys
bun run i18n:sync
...
# Refresh Apple terminology data when the AppleGlot source changes
bun run i18n:apple-glossary
...
`i18n:sync` copies English values into missing locale keys, while `i18n:sync:mark-todo` marks those keys with `[TODO]` so translators can spot them. Use the TODO variant for new user-facing copy unless you are intentionally preserving English text.

`i18n:audit` is the terminology gate. It checks missing and extra keys, `[TODO]` leftovers, placeholder parity, CLDR plural forms, required key translations, Traditional Chinese account terminology, and Apple UI terminology pulled from `scripts/apple-ui-terminology.ts`. Use `bun run i18n:audit:fix` only for missing or obsolete keys, required translation overrides, plural backfills, and terminology drift that the script can repair safely, then rerun the audit.

Apple terminology comes from `scripts/apple-ui-terminology-data.ts`, generated by `bun run i18n:apple-glossary` from AppleGlot macOS glossary files. When a term needs product-specific wording, add a scoped override to `APPLE_UI_CONTEXTUAL_TERMINOLOGY` in `scripts/apple-ui-terminology.ts` or `REQUIRED_KEY_TRANSLATIONS` in `scripts/audit-translations.ts` rather than weakening the global glossary rule.
