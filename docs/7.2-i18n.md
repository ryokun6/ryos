...
# Machine-translate [TODO] keys.
# Cursor Cloud already injects GOOGLE_GENERATIVE_AI_API_KEY.
# Local maintainer machines only: export it from .env.local first.
...
`i18n:translate` reads `GOOGLE_GENERATIVE_AI_API_KEY` from the process environment. Cursor Cloud already injects it. Local maintainer machines should export it from `.env.local` first because standalone Bun scripts do not auto-load that file. Review machine translations before committing because UI length, tone, and product terminology still need maintainer judgment.
