...
When adding help rows, keep the source `helpItems` array and the app's help key list in the same order. Maps uses `src/apps/maps/helpKeys.ts` plus `tests/test-maps-help-i18n.test.ts`; Calculator and Internet Explorer use the same alignment pattern in `src/apps/{calculator,internet-explorer}/helpKeys.ts` and their matching `tests/test-*-help-i18n.test.ts` files.
