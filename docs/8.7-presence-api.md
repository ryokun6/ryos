...
|-----------|------|----------|-------------|
| `search` | string | Yes | Username query (min 2 characters) |
...
**Notes:**
- Searches are substring matches against usernames.
...

{
  ...
  "systemState": {
    "chatRoomContext": {
      ...
      "mentionedMessage": "alice: @ryo what's up?"
    }
  }
}
```
...
| `systemState` | object | No | Optional system state for context |
| `systemState.chatRoomContext.recentMessages` | string | No | Recent chat history for context |
| `systemState.chatRoomContext.mentionedMessage` | string | No | The message that mentioned @ryo |
...
```json
{
  "message": {
    ...
    "roomId": "general",
    "username": "ryo",
    "content": "Hey! Just hanging out in the chat. How can I help you today?",
...
|-----------|-------|--------|
| Authenticated | 5 requests | 1 minute (per user) |
