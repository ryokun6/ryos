...
} from "@/utils/sync/engine/domains/entityMerge";
...
export function serializeCalendarSyncSnapshot(): CalendarSnapshotData {
...
export function applyCalendarSyncSnapshot(data: CalendarSnapshotData): void {
  ...
  useCalendarStore.setState({
    ...
    todos: filterDeletedIds(data.todos, effectiveDeletedTodoIds, (todo) => todo.id),
  });
}

export function mergeCalendarSyncSnapshots(
  ...
  return {
    ...
    todos: mergeItemsById(
      filterDeletedIds(local.todos, mergedDeletedTodos, (todo) => todo.id),
      filterDeletedIds(remote.todos, mergedDeletedTodos, (todo) => todo.id)
    ),
