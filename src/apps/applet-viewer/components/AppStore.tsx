...
import { Trash2, Star, ArrowLeft, ChevronUp, ChevronDown } from "lucide-react";
...
export function AppStore({ theme, sharedAppletId }: AppStoreProps) {
  ...
  const [activeAppletIndex, setActiveAppletIndex] = useState<number | null>(
    null,
  );
  const [selectedAppletContent, setSelectedAppletContent] =
    useState<string>("");
  const [isSharedApplet, setIsSharedApplet] = useState(false);
  const [sharedDetailApplet, setSharedDetailApplet] = useState<Applet | null>(
    null,
  );
  ...
  const fetchApplets = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const sortedApplets = (data.applets || []).sort(
          (a: Applet, b: Applet) => {
  ...
  useEffect(() => {
    if (!sharedAppletId) return;

    setSharedDetailApplet(null);
    setActiveAppletIndex(null);
    ...
    const fetchSharedApplet = async () => {
      try {
        const response = await fetch(
          `/api/share-applet?id=${encodeURIComponent(sharedAppletId)}`,
        );
        if (response.ok) {
          const data = await response.json();
          const applet: Applet = {
          ...
          setSharedDetailApplet(applet);
          setActiveAppletIndex(0);
  ...
  const handleAppletClick = async (applet: Applet) => {
    ...
    if (installed) {
      ...
      if (installedApplet) {
        ...
        try {
          const response = await fetch(
            `/api/share-applet?id=${encodeURIComponent(applet.id)}`,
          );
          if (response.ok) {
            const data = await response.json();
      ...
      const indexInFeed = detailFeed.findIndex((item) => item.id === applet.id);
      if (indexInFeed === -1) {
        setSharedDetailApplet(applet);
        setActiveAppletIndex(0);
      } else {
  ...
  const handleDelete = async (appletId: string) => {
    ...
    try {
      const response = await fetch(
        `/api/share-applet?id=${encodeURIComponent(appletId)}`,
        {
          method: "DELETE",
          headers: {
  ...
  }, [updatesAvailable, featuredApplets, allApplets, installedApplets]);

  const augmentedFeed = useMemo(() => {
    if (
      sharedDetailApplet &&
      !detailFeed.some((applet) => applet.id === sharedDetailApplet.id)
    ) {
      return [sharedDetailApplet, ...detailFeed];
    }
    return detailFeed;
  }, [detailFeed, sharedDetailApplet]);

  useEffect(() => {
    if (!sharedDetailApplet) return;

    const indexInFeed = detailFeed.findIndex(
  ...
  useEffect(() => {
    if (
      activeAppletIndex !== null &&
      activeAppletIndex >= augmentedFeed.length
    ) {
      setActiveAppletIndex(
        augmentedFeed.length > 0 ? augmentedFeed.length - 1 : null,
      );
    }
  }, [activeAppletIndex, augmentedFeed]);

  const currentApplet =
    activeAppletIndex !== null ? augmentedFeed[activeAppletIndex] : null;
  const feedLength = augmentedFeed.length;
  ...
  useEffect(() => {
    ...
    if (isSharedApplet && selectedAppletContent) {
      return;
    }

    setSelectedAppletContent("");

    fetch(`/api/share-applet?id=${encodeURIComponent(currentApplet.id)}`)
      .then((response) => {
        if (response.ok) {
  ...
  }, [currentApplet, isSharedApplet]);

  const hasPreviousApplet = activeAppletIndex !== null && activeAppletIndex > 0;
  const hasNextApplet =
    activeAppletIndex !== null && activeAppletIndex < feedLength - 1;

  const goToNextApplet = useCallback(() => {
    setActiveAppletIndex((prev) => {
  ...
  const goToPreviousApplet = useCallback(() => {
    setActiveAppletIndex((prev) => {
      if (prev === null) return prev;
      const nextIndex = prev - 1;
      if (nextIndex < 0) {
  ...
  const handleTouchEnd = useCallback(
    (event: ReactTouchEvent<HTMLDivElement>) => {
      ...
      if (duration <= MAX_DURATION && Math.abs(deltaY) > SWIPE_DISTANCE) {
        if (deltaY < 0) {
          goToNextApplet();
        } else {
          goToPreviousApplet();
    ...
    [goToNextApplet, goToPreviousApplet],
  );
  ...
  const handleWheel = useCallback(
    ...
    [goToNextApplet, goToPreviousApplet, hasNextApplet, hasPreviousApplet],
  );

  useEffect(() => {
    if (activeAppletIndex === null) return;
  ...
  }, [activeAppletIndex, goToNextApplet, goToPreviousApplet]);
          ...
          {isAdmin && (
            <div className="flex items-center gap-0">
              <button
                ...
                title={
                  applet.featured ? "Remove from featured" : "Add to featured"
                }
              >
                <Star
  ...
  // Detail view for applet feed
  if (currentApplet) {
    const displayName =
      currentApplet.title || currentApplet.name || "Untitled Applet";
    const displayIcon = currentApplet.icon || "📱";
    const installed = isAppletInstalled(currentApplet.id);
    const updateAvailable = needsUpdate(currentApplet);
      ...
      <>
        ...
        >
          ...
          >
            ...
            <Button
              ...
              onClick={() => {
                if (installed) {
                  if (updateAvailable) {
                    handleInstall(currentApplet);
                  } else {
                    handleAppletClick(currentApplet);
                  }
                } else {
                  handleInstall(currentApplet);
                }
          ...
          <div className="relative flex-1 overflow-hidden bg-white">
            {hasPreviousApplet && (
              <div className="pointer-events-none absolute top-2 left-0 right-0 flex justify-center z-20">
                <button
                  type="button"
                  className="pointer-events-auto inline-flex items-center gap-2 rounded-full bg-black/60 px-3 py-1 text-[11px] text-white backdrop-blur-sm transition hover:bg-black/75"
                  onClick={(event) => {
                ...
                >
                  ...
                  <span className="font-medium">Previous app</span>
                </button>
              </div>
            )}
            ...
            {hasNextApplet && (
              <div className="pointer-events-none absolute bottom-2 left-0 right-0 flex justify-center z-20">
                ...
                >
                  <span className="font-medium">Next app</span>
