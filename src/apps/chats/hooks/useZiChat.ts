...
export function useZiChat({
  ...
  // Create a separate AI chat hook for @zi mentions in chat rooms
  const {
    messages: ziMessages,
    status,
    stop: stopZi,
  ...
  const isZiLoading = status === "streaming" || status === "submitted";

  const handleZiMention = useCallback(
    async (messageContent: string) => {
      ...
      // Call server to generate and insert a @zi reply using authenticated request
      ...
      await fetch(`/api/chat-rooms?action=generateZiReply`, {
        method: "POST",
        headers,
        body: JSON.stringify({
          roomId: currentRoomId,
  ...
  const detectAndProcessMention = useCallback(
    (input: string): { isMention: boolean; messageContent: string } => {
      if (input.startsWith("@zi ")) {
        // Extract the message content after @zi
        const messageContent = input.substring(4).trim();
        return { isMention: true, messageContent };
      } else if (input === "@zi") {
        // If they just typed @zi without a message, treat it as a nudge
