/**
 * Fuzzy text-matching helpers shared by the chats AI tools.
 *
...
export const isLooseSubsequence = (target: string, pattern: string): boolean => {
  if (pattern.length === 0) {
    return true;
  }
...
export const bestSubstringDistance = (text: string, query: string): number => {
