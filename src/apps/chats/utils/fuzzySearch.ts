/**
 * Fuzzy search utilities used by chats tool handlers (file/applet/track
 * lookup, karaoke matching, etc.).
 *
 * Public API:
 *   - normalizeSearchText(value)   → NFKD-normalized, diacritic-stripped, lowercased
 *   - computeMatchScore(text, query, tokens)
 *       → 0..1 similarity score combining substring, subsequence, and
 *         Levenshtein-window matches plus per-token bonuses.
 *   - deriveScoreThreshold(queryLength)
 *       → recommended minimum score for accepting a match, scaled by query length.
 *
 * The lower-level primitives (`isLooseSubsequence`, `levenshteinDistance`,
 * `bestSubstringDistance`) are kept module-private; callers should compose
 * via `computeMatchScore`.
 */
