...
export interface WorkIntelInstance {
  instanceId: string;
...
export interface WorkIntelStoreState {
  // Instance management
  instances: Record<string, WorkIntelInstance>;
  ...
  updateInstance: (
    instanceId: string,
    updates: Partial<Omit<WorkIntelInstance, "instanceId">>
  ) => void;
  getInstanceByPath: (path: string) => WorkIntelInstance | null;
  getInstanceIdByPath: (path: string) => string | null;
  getForegroundInstance: () => WorkIntelInstance | null;
...
const CURRENT_WORKINTEL_STORE_VERSION = 2;

export const useWorkIntelStore = create<WorkIntelStoreState>()(
  persist(
    (set, get) => ({
      // Instance state
      instances: {},
      ...
      getForegroundInstance: () => {
        ...
        if (!foregroundInstance || foregroundInstance.appId !== "workintel") {
          return null;
        }
      ...
      migrate: (persistedState: unknown, version: number) => {
        ...
        if (version < 2) {
          ...
          const migratedState: Partial<WorkIntelStoreState> = {
            instances: {},
    ...
    {
      name: "ryos:workintel",
      version: CURRENT_WORKINTEL_STORE_VERSION,
      migrate: (persistedState: unknown, version: number) => {
        // Migrate from v1 to v2 (single window to multi-instance)
        if (version < 2) {
          const oldState = persistedState as {
