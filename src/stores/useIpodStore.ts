...
import { parseMediaTitle } from "@/api/media";
import {
  clearSongCachedData,
  fetchSongLyrics,
  listSongs,
  updateSongById,
  type SongLyricsSource,
} from "@/api/songs";
...
async function saveLyricOffsetToServer(
  ...
  try {
    const data = await updateSongById<{ lyricOffset: number }>(
      trackId,
      { lyricOffset },
  ...
  persist(
    (set, get) => ({
      ...
      clearLyricsCache: () => {
        ...
        if (currentTrack?.id) {
          clearSongCachedData(currentTrack.id, {
            clearTranslations: true,
            clearFurigana: true,
            clearSoramimi: true,
          }).catch((err) => {
      ...
      addTrackFromVideoId: async (urlOrId: string, autoPlay: boolean = true): Promise<Track | null> => {
        ...
        try {
          const fetchData = await fetchSongLyrics<{
            metadata?: {
              title?: string;
              artist?: string;
              album?: string;
        ...
        if (!trackInfo.lyricsSource) {
          ...
          try {
            const parsedData = await parseMediaTitle({
              title: rawTitle,
              author_name: authorName,
            });
            trackInfo.title = parsedData.title || rawTitle;
            trackInfo.artist = parsedData.artist || undefined;
            trackInfo.album = parsedData.album || undefined;
          } catch (error) {
          ...
          if (tracksNotInDefaultLibrary.length > 0) {
            ...
            try {
              ...
              const data = await listSongs<FetchedSongMetadata>({
                ids: idsToFetch ? idsToFetch.split(",") : [],
                include: "metadata",
              });
