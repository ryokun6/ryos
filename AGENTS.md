...
- `S3_PROXY_BLOBS` - Set to `1`/`true` (S3 backend only) to stream cloud-sync blob and backup uploads/downloads through the same-origin API (`/api/sync/blob-proxy`) instead of direct browser→bucket presigned requests. Use this when the object-storage bucket's CORS allowlist can't cover every origin the app is served from (self-hosted domain, Electron app, `localhost`), which otherwise fails with "Upload failed before an HTTP response was received". Do **not** enable on Vercel Serverless Functions (~4.5MB request-body cap); it's for plain Bun / Coolify / VPS hosts.
