...
- **Pre-existing lint errors**: There is one pre-existing `@typescript-eslint/no-explicit-any` error in `src/apps/winamp/components/WinampAppComponent.tsx` and six `no-useless-escape` errors in `src/components/shared/ToolInvocationMessage.tsx`. Do not attempt to fix them unless asked.
