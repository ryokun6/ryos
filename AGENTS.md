...
### Optional Features
- `RESEND_API_KEY` - Resend API key enabling the account-recovery **email** channel (verify recovery email + email-delivered password-reset codes). When unset, email recovery is unavailable and account recovery falls back to a linked Telegram account.
- `RECOVERY_EMAIL_FROM` - `From` address for recovery emails (e.g. `ryOS <noreply@os.ryo.lu>`). Required alongside `RESEND_API_KEY` for the email channel to activate.
