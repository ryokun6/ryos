...
- **Redis/realtime on the cloud VM must be Upstash REST + Pusher; do NOT use the production Coolify values.** The cloud-agent secret set is configured with `REDIS_PROVIDER=upstash` (and **no** `REDIS_URL`, **no** `REALTIME_PROVIDER`), so a bare `bun run dev` / `bun run test:api` "just works": Redis resolves to Upstash REST and realtime defaults to Pusher (cluster `us3`), both reachable and validated. **Do not add `REDIS_URL` or set `REALTIME_PROVIDER=local` here** — the production `REDIS_URL` is an internal Docker host (e.g. `redis://t0408...:6379`) that does not resolve from this VM (`getent hosts` fails), `getRedisBackend()` (`api/_utils/redis.ts`) prefers `REDIS_URL` over Upstash so it would break core storage, and `local` realtime needs that same unreachable host for pub/sub. If storage suddenly fails to connect, check that `REDIS_PROVIDER` is still `upstash` and that no `REDIS_URL` leaked back into the env (`REDIS_PROVIDER=redis-url/redis/standard` demands `REDIS_URL` and will crash the API on boot).
