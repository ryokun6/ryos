...
Several test suites are pure unit/wiring tests that run without a live server:
```bash
bun run tests/run-all-tests.ts chat-notification   # Chat notification logic
bun run tests/run-all-tests.ts pusher               # Pusher client wiring
bun run tests/run-all-tests.ts chat-broadcast       # Chat broadcast wiring
bun run tests/run-all-tests.ts chat-store           # Chat store guards
bun run tests/run-all-tests.ts chat-hook            # Chat hook lifecycle
```

### API Tests (server required)

The remaining tests (`bun run test`) require a running server at `http://localhost:3000` and Redis credentials.
...
- **Bun path**: Bun is installed at `~/.bun/bin/bun`. The update script ensures it's on PATH via `~/.bashrc`.
- **No `.env` file ships with the repo**: Environment variables must be configured manually or pulled from Vercel (`vercel env pull`). The frontend SPA works fully without any env vars — only API features require them.
