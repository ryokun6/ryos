...
- **Secrets are available two ways and `bun run dev` has full functionality out of the box.** They are (a) injected as environment variables on the cloud VM and (b) mirrored into a gitignored **`.env.local`** that is baked into the VM snapshot (Bun auto-loads `.env.local`). This covers Redis (Upstash REST: `REDIS_KV_REST_API_URL`/`REDIS_KV_REST_API_TOKEN` with `REDIS_PROVIDER=upstash`), Pusher, AI keys (OpenAI/Anthropic/Google), and Apple MapKit/MusicKit. Do **not** commit `.env.local`. If you add a new secret, set it in the run's Secrets panel (injected) — the snapshot `.env.local` is a convenience fallback, not the source of truth.
